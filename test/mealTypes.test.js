...
test('buildDeck with no meal type filter returns dishes from the full pool', () => {
  const deck = buildDeck(20, []);
  assert.ok(deck.length > 0);
});

test('buildDeck filtered to dessert only returns dessert dishes', () => {
  const deck = buildDeck(20, ['dessert']);
  assert.ok(deck.length > 0);
...
test('buildDeck filtered to breakfast+dessert only returns matching dishes', () => {
  const deck = buildDeck(20, ['breakfast', 'dessert']);
  assert.ok(deck.length > 0);
  for (const dish of deck) {
